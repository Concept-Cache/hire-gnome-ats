...
const STATUS_LABELS = new Map(CLIENT_STATUS_OPTIONS.map((option) => [option.value, option.label]));
...
export function normalizeClientAdvancedCriteria(raw) {
	if (!Array.isArray(raw)) return [];
	return raw.map(normalizeCriterion).filter(Boolean);
}

export function getClientAdvancedFieldDefinitions({ ownerOptions = [], divisionOptions = [] } = {}) {
...
export function getClientAdvancedOperatorOptions(fieldKey) {
	const definition = fieldDefinitionFor(fieldKey);
	return definition ? getOperatorOptionsForType(definition.type) : [];
}

export function createDefaultClientAdvancedCriterion() {
	return {
		field: 'name',
...
export function isClientAdvancedCriterionComplete(criterion) {
...
export function evaluateClientAdvancedCriteria(row, rawCriteria) {
	const criteria = normalizeClientAdvancedCriteria(rawCriteria).filter(isClientAdvancedCriterionComplete);
...
function operatorLabelFor(fieldKey, operator) {
	const options = getClientAdvancedOperatorOptions(fieldKey);
...
export function summarizeClientAdvancedCriterion(rawCriterion) {
	const criterion = normalizeCriterion(rawCriterion);
	if (!criterion || !isClientAdvancedCriterionComplete(criterion)) return '';
