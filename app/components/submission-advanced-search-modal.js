...
import {
	createDefaultSubmissionAdvancedCriterion,
	getSubmissionAdvancedFieldDefinitions,
	getSubmissionAdvancedOperatorOptions,
	isSubmissionAdvancedCriterionComplete,
...
function nextCriterionForField(fieldDefinitions, fieldKey) {
	...
	const defaultOperator = getSubmissionAdvancedOperatorOptions(definition?.key || '')[0]?.value || 'is';
...
export default function SubmissionAdvancedSearchModal({
	open = false,
	criteria = [],
	statusOptions = [],
	submitterOptions = [],
	...
	const fieldDefinitions = useMemo(
		() => getSubmissionAdvancedFieldDefinitions({ statusOptions, submitterOptions }),
		[statusOptions, submitterOptions]
	);
	const [draftCriteria, setDraftCriteria] = useState(() => normalizeSubmissionAdvancedCriteria(criteria));

	useEffect(() => {
		if (!open) return;
		setDraftCriteria(normalizeSubmissionAdvancedCriteria(criteria));
	}, [criteria, open]);

	const completeCriteriaCount = draftCriteria.filter(isSubmissionAdvancedCriterionComplete).length;
	const hasIncompleteCriteria = draftCriteria.some((criterion) => !isSubmissionAdvancedCriterionComplete(criterion));
	...
	function addCriterion() {
		setDraftCriteria((current) => [...current, createDefaultSubmissionAdvancedCriterion()]);
	}
	...
	function applyCriteria() {
		const nextCriteria = normalizeSubmissionAdvancedCriteria(draftCriteria).filter(
			isSubmissionAdvancedCriterionComplete
		);
	...
	return (
		<div className="confirm-overlay" role="presentation">
			<div className="job-order-advanced-search-modal" role="dialog" aria-modal="true" aria-labelledby="submission-advanced-search-title">
				<div className="job-order-advanced-search-modal-head">
					<div>
						<p className="job-order-advanced-search-eyebrow">Submissions</p>
						<h3 id="submission-advanced-search-title">Advanced Search</h3>
				...
				<div className="job-order-advanced-search-body">
					<div className="job-order-advanced-search-rows">
						{draftCriteria.length === 0 ? (
							<div className="job-order-advanced-search-empty">No advanced filters yet. Add a filter to narrow the list beyond quick search.</div>
						) : null}
						{draftCriteria.map((criterion, index) => {
							const fieldDefinition =
								fieldDefinitions.find((field) => field.key === criterion.field) || fieldDefinitions[0];
							const operatorOptions = getSubmissionAdvancedOperatorOptions(fieldDefinition.key);
							return (
