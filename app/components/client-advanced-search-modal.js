...
import {
	createDefaultClientAdvancedCriterion,
	getClientAdvancedFieldDefinitions,
	getClientAdvancedOperatorOptions,
	isClientAdvancedCriterionComplete,
...
function nextCriterionForField(fieldDefinitions, fieldKey) {
	...
	const defaultOperator = getClientAdvancedOperatorOptions(definition?.key || '')[0]?.value || 'is';
...
export default function ClientAdvancedSearchModal({
	open = false,
	criteria = [],
	ownerOptions = [],
	divisionOptions = [],
	...
	const fieldDefinitions = useMemo(
		() => getClientAdvancedFieldDefinitions({ ownerOptions, divisionOptions }),
		[divisionOptions, ownerOptions]
	);
	const [draftCriteria, setDraftCriteria] = useState(() => normalizeClientAdvancedCriteria(criteria));

	useEffect(() => {
		if (!open) return;
		setDraftCriteria(normalizeClientAdvancedCriteria(criteria));
	}, [criteria, open]);

	const completeCriteriaCount = draftCriteria.filter(isClientAdvancedCriterionComplete).length;
	const hasIncompleteCriteria = draftCriteria.some((criterion) => !isClientAdvancedCriterionComplete(criterion));
	...
	function addCriterion() {
		setDraftCriteria((current) => [...current, createDefaultClientAdvancedCriterion()]);
	}
	...
	function applyCriteria() {
		const nextCriteria = normalizeClientAdvancedCriteria(draftCriteria).filter(isClientAdvancedCriterionComplete);
	...
	return (
		<div className="confirm-overlay" role="presentation">
			<div
				...
				role="dialog"
				aria-modal="true"
				aria-labelledby="client-advanced-search-title"
			>
				<div className="job-order-advanced-search-modal-head">
					<div>
						<p className="job-order-advanced-search-eyebrow">Clients</p>
						<h3 id="client-advanced-search-title">Advanced Search</h3>
				...
				<div className="job-order-advanced-search-body">
					<div className="job-order-advanced-search-rows">
						...
						{draftCriteria.map((criterion, index) => {
							...
							const operatorOptions = getClientAdvancedOperatorOptions(fieldDefinition.key);
