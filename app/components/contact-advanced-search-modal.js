...
import {
	createDefaultContactAdvancedCriterion,
	getContactAdvancedFieldDefinitions,
	getContactAdvancedOperatorOptions,
	isContactAdvancedCriterionComplete,
...
function nextCriterionForField(fieldDefinitions, fieldKey) {
	...
	const defaultOperator = getContactAdvancedOperatorOptions(definition?.key || '')[0]?.value || 'is';
...
export default function ContactAdvancedSearchModal({
	open = false,
	criteria = [],
	clientOptions = [],
	ownerOptions = [],
	...
	const fieldDefinitions = useMemo(
		() => getContactAdvancedFieldDefinitions({ clientOptions, ownerOptions, sourceOptions, divisionOptions }),
		[clientOptions, divisionOptions, ownerOptions, sourceOptions]
	);
	const [draftCriteria, setDraftCriteria] = useState(() => normalizeContactAdvancedCriteria(criteria));

	useEffect(() => {
		if (!open) return;
		setDraftCriteria(normalizeContactAdvancedCriteria(criteria));
	}, [criteria, open]);

	const completeCriteriaCount = draftCriteria.filter(isContactAdvancedCriterionComplete).length;
	const hasIncompleteCriteria = draftCriteria.some((criterion) => !isContactAdvancedCriterionComplete(criterion));
	...
	function addCriterion() {
		setDraftCriteria((current) => [...current, createDefaultContactAdvancedCriterion()]);
	}
	...
	function applyCriteria() {
		const nextCriteria = normalizeContactAdvancedCriteria(draftCriteria).filter(isContactAdvancedCriterionComplete);
	...
	return (
		<div className="confirm-overlay" role="presentation">
			<div
				className="job-order-advanced-search-modal"
				role="dialog"
				aria-modal="true"
				aria-labelledby="contact-advanced-search-title"
			>
				<div className="job-order-advanced-search-modal-head">
					<div>
						<p className="job-order-advanced-search-eyebrow">Contacts</p>
						<h3 id="contact-advanced-search-title">Advanced Search</h3>
						<p className="job-order-advanced-search-copy">
							Build structured filters for the list. Quick search stays separate.
						</p>
					</div>
					<button
						type="button"
				...
				<div className="job-order-advanced-search-body">
					<div className="job-order-advanced-search-rows">
						...
						{draftCriteria.map((criterion, index) => {
							...
							const operatorOptions = getContactAdvancedOperatorOptions(fieldDefinition.key);
					...
					<div className="job-order-advanced-search-footer-note">
						...
						<span>
							{hasIncompleteCriteria
								? 'Complete each filter row before applying.'
