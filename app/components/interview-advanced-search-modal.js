...
import {
	createDefaultInterviewAdvancedCriterion,
	getInterviewAdvancedFieldDefinitions,
	getInterviewAdvancedOperatorOptions,
	isInterviewAdvancedCriterionComplete,
...
function nextCriterionForField(fieldDefinitions, fieldKey) {
	...
	const defaultOperator = getInterviewAdvancedOperatorOptions(definition?.key || '')[0]?.value || 'is';
...
export default function InterviewAdvancedSearchModal({
	open = false,
	criteria = [],
	typeOptions = [],
	statusOptions = [],
	...
	const fieldDefinitions = useMemo(
		() => getInterviewAdvancedFieldDefinitions({ typeOptions, statusOptions }),
		[typeOptions, statusOptions]
	);
	const [draftCriteria, setDraftCriteria] = useState(() => normalizeInterviewAdvancedCriteria(criteria));

	useEffect(() => {
		if (!open) return;
		setDraftCriteria(normalizeInterviewAdvancedCriteria(criteria));
	}, [criteria, open]);

	const completeCriteriaCount = draftCriteria.filter(isInterviewAdvancedCriterionComplete).length;
	const hasIncompleteCriteria = draftCriteria.some((criterion) => !isInterviewAdvancedCriterionComplete(criterion));
	...
	function addCriterion() {
		setDraftCriteria((current) => [...current, createDefaultInterviewAdvancedCriterion()]);
	}
	...
	function applyCriteria() {
		onApply?.(normalizeInterviewAdvancedCriteria(draftCriteria).filter(isInterviewAdvancedCriterionComplete));
	}

	return (
		<div className="confirm-overlay" role="presentation">
			<div className="job-order-advanced-search-modal" role="dialog" aria-modal="true" aria-labelledby="interview-advanced-search-title">
				<div className="job-order-advanced-search-modal-head">
					<div>
						<p className="job-order-advanced-search-eyebrow">Interviews</p>
						<h3 id="interview-advanced-search-title">Advanced Search</h3>
				...
				<div className="job-order-advanced-search-body">
					<div className="job-order-advanced-search-rows">
						...
						{draftCriteria.map((criterion, index) => {
							...
							const operatorOptions = getInterviewAdvancedOperatorOptions(fieldDefinition.key);
							return (
