...
import {
	createDefaultCandidateAdvancedCriterion,
	getCandidateAdvancedFieldDefinitions,
	getCandidateAdvancedOperatorOptions,
	isCandidateAdvancedCriterionComplete,
...
function nextCriterionForField(fieldDefinitions, fieldKey) {
	...
	const defaultOperator = getCandidateAdvancedOperatorOptions(definition?.key || '')[0]?.value || 'is';
...
export default function CandidateAdvancedSearchModal({
	open = false,
	criteria = [],
	ownerOptions = [],
	sourceOptions = [],
	...
	const fieldDefinitions = useMemo(
		() => getCandidateAdvancedFieldDefinitions({ ownerOptions, sourceOptions, divisionOptions }),
		[divisionOptions, ownerOptions, sourceOptions]
	);
	const [draftCriteria, setDraftCriteria] = useState(() => normalizeCandidateAdvancedCriteria(criteria));

	useEffect(() => {
		if (!open) return;
		setDraftCriteria(normalizeCandidateAdvancedCriteria(criteria));
	}, [criteria, open]);

	const completeCriteriaCount = draftCriteria.filter(isCandidateAdvancedCriterionComplete).length;
	const hasIncompleteCriteria = draftCriteria.some((criterion) => !isCandidateAdvancedCriterionComplete(criterion));
	...
	function addCriterion() {
		setDraftCriteria((current) => [...current, createDefaultCandidateAdvancedCriterion()]);
	}
	...
	function applyCriteria() {
		const nextCriteria = normalizeCandidateAdvancedCriteria(draftCriteria).filter(
			isCandidateAdvancedCriterionComplete
		);
	...
	return (
		<div className="confirm-overlay" role="presentation">
			<div
				...
				role="dialog"
				aria-modal="true"
				aria-labelledby="candidate-advanced-search-title"
			>
				<div className="job-order-advanced-search-modal-head">
					<div>
						<p className="job-order-advanced-search-eyebrow">Candidates</p>
						<h3 id="candidate-advanced-search-title">Advanced Search</h3>
				...
				<div className="job-order-advanced-search-body">
					<div className="job-order-advanced-search-rows">
						...
						{draftCriteria.map((criterion, index) => {
							...
							const operatorOptions = getCandidateAdvancedOperatorOptions(fieldDefinition.key);
